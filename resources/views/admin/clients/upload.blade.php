...
@section('content')
<div class="max-w-6xl mx-auto p-6 bg-white dark:bg-gray-800 rounded-md shadow-md">
    ...
    <form action="{{ route('admin.clients.upload', $client->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-2 gap-6">
        @csrf
        ...
        @foreach ($fields as $name => $label)
            <div class="flex flex-col">
                ...
                <input id="{{ $name }}" type="file" name="{{ $name }}" accept="application/pdf" required
